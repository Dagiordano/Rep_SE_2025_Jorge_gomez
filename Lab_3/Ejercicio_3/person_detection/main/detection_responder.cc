...
static void init_detection_led(void) {
  if (!led_initialized) {
    gpio_config_t io_conf = {};
    io_conf.intr_type = GPIO_INTR_DISABLE;
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pin_bit_mask = (1ULL << LED_DETECTION_PIN);
    io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
    io_conf.pull_up_en = GPIO_PULLUP_DISABLE;
    gpio_config(&io_conf);
    gpio_set_level((gpio_num_t)LED_DETECTION_PIN, 0); // Start with LED off
...
void RespondToDetection(float person_score, float no_person_score) {
  ...
  // Initialize LED pin if not already done
  init_detection_led();
  
  // Control physical LED based on person detection score
  if (person_score_int > 85) {
    gpio_set_level((gpio_num_t)LED_DETECTION_PIN, 1); // Turn LED on
  } else {
    gpio_set_level((gpio_num_t)LED_DETECTION_PIN, 0); // Turn LED off
  }
